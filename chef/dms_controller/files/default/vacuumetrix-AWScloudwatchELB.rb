...
begin
  if File.readable?($timestampfile) then 
    File.open($timestampfile,"r") { |file| startTime = Marshal.load(file) } 
  end
rescue => ex
  puts "Bad timestamp data in #{$timestampfile}, #{ex}, skipping"
end
...
lbs.each do |table|
  metricNames.each do |metricName, statistic|
    ...
    responses.each do |response|
      metricpath = "AWScloudwatch.ELB." + table.gsub("-",".") + "." + metricName + "." + statistic
      begin
...
File.open($timestampfile,"w") { |file| Marshal.dump(endTime, file) }
...
lbs.each do |table|
  metricNames.each do |statistic, metricName|
    ...
    metricpath = "AWScloudwatch.ELB." + table.gsub("-",".") + "." + metricName + "_" + statistic
    responses.each do |response|
      begin
        metricvalue = response[statistic]
        metrictimestamp = response["Timestamp"].to_i.to_s
