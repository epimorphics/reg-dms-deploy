...
scp -Cq $SSH_FLAGS -i $AWS_KEY $imageFile ubuntu@${IP}:$remoteDir/$fileName

ssh $SSH_FLAGS -i $AWS_KEY -l ubuntu $IP "sudo bash /usr/local/bin/dbinstall $remoteDir/$fileName"
